...
public class RequestLogRestIntegrationTest {

    private static final String API_PATH = "/api/request-logs";
    ...
    @Test
    public void shouldIndexRequestLogs() throws Exception {
        ...
        RequestLog requestLog = requestLogRepository.save(RequestLog.builder()
                .terminal(terminal)
                .request("X")
                .build());

        List<RequestLogDTO> response = RestAssured.given()
                ...
                .then().log().all()
                .statusCode(200)
                .extract().as(new TypeRef<List<RequestLogDTO>>() {
                });

        Set<UUID> responseIds = response.stream()
                .map(RequestLogDTO::getId)
                .collect(Collectors.toSet());

        assertThat(responseIds.size(), is(response.size()));
        assertThat(responseIds, hasItem(requestLog.getId()));
    }

    @Test
    public void shouldShowRequestLog() throws Exception {
        Mockito.when(securityService.getUserId())
                .thenReturn(USER_ID);
        ...
        RequestLogDTO response = RestAssured.given()
                .baseUri("http://localhost:" + port)
                .contentType(ContentType.JSON)
                .when().log().all()
                .get(API_PATH + "/{id}", requestLog.getId())
                .then().log().all()
                .statusCode(200)
                .extract().as(RequestLogDTO.class);

        assertThat(response, is(notNullValue()));
        assertThat(response.getId(), is(requestLog.getId()));
    }
}
